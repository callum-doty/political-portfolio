...
|---|---|---|---|---|---|
| 2022 | 68 days | $-2.18$ | 1.53 | $-1.42$ | Yes |
| 2024 | 65 days | $-1.95$ | 1.50 | $-1.30$ | Yes |

Both realized moves are within 2 standard deviations of the calibration (not a rejection), but both sit at $z\approx-1.3$ to $-1.4$ — larger than typical, and **in the same direction in both cycles** (the generic ballot moved toward Republicans by a similar amount in the final ~2 months of both 2022 and 2024). With only two cycles this cannot be treated as a finding, but it is a pattern worth flagging rather than averaging away: a symmetric, zero-mean shock model may understate a real late-cycle directional tendency, and this is a natural thing for a future cycle's data point to confirm or refute, not something to resolve here.
...
§8.2 also reports a correction found immediately after this result was first drafted: the model first tested was missing two of $P$'s three calibrated components ($\hat\eta$ and $G_t$, neither of which entered that first run of `solve_bellman_lsm.py`). Rather than leave that gap open, `docs/theta_followup_plan.md` §0 specified the fix and §4 implements and re-runs it: $\hat\eta$ now discounts the deploy branch's linearized gradient by tier, and $G_t$ is simulated as a fifth continuation-value regression feature, respecting the alpha3 scope boundary. A third, related mechanism — giving $\eta$ something to react to on the *wait* branch via candidate-committee floor growth — was found not to be implementable from data that exists anywhere in this repository, and is reported as a real, likely-permanent limitation rather than approximated. **The corrected re-run's top-line conclusion is unchanged: both eta-brackets still recommend full deployment at $t=0$** — but the correction is not vacuous: the 2024 bracket's $\Theta(0)$ moved from a large negative number to $-0.039$ (an order of magnitude closer to zero), and that bracket's mid-cycle patience premium widened and strengthened (five consecutive periods of positive $\Theta$, peaking at $+0.123$ expected seats 56 days out). A cheap, separate check (an Ornstein-Uhlenbeck-with-drift fit on the pooled four-cycle generic-ballot series, `docs/theta_followup_plan.md` §4) found the drift term statistically indistinguishable from zero at the live decision's horizon, closing off — for now — the cleanest version of the "current environment is unusually favorable" hypothesis as an explanation for anything in this result. What remains open, and is sequenced as this paper's next step, is whether the binary deploy-everything-or-hold-everything framing is itself too coarse to express a genuine small reserve, now that the model testing that question is the complete one this paper's Sections 4–7 specified, not an incomplete stand-in for it.

---

*Draft status: Sections 1–7.1 and 9–10 are conceptually and empirically complete: real, fitted estimates for $\hat\eta(\text{tier})$ (§4.4) and $\sigma_G(\Delta t)$ (§5.3), a simulator assembled and validated against held-out 2022/2024 data (§5.5) and a four-part Monte Carlo self-consistency gate (§7.1). **Sections 7.2 and 8.2 report the completed Longstaff–Schwartz backward induction**, run against the live 2026 state, corrected and re-run after an initial draft found that neither $\hat\eta$ nor $G_t$ actually entered the tested model (`docs/theta_followup_plan.md` §0 specifies the fix; §4 is the implementation and re-run log). With the correction applied — $\hat\eta$ discounting the deploy branch's gradient by tier, $G_t$ live as a fifth, correctly-null regression feature, and one remaining real data gap (wait-branch floor growth) reported rather than faked — $\Theta(t)$ is negative (deploy favored) at $t=0$ in both eta-brackets ($-0.517$ and $-0.039$), with a wider, somewhat stronger mid-cycle positive-patience window in the 2024 bracket than the pre-correction run showed. The closing comparison against Paper II's 65.5% baseline still lands at >30% on §8.1's pre-registered scale, since the action prescribed at $t=0$ (full deployment) is unchanged by the correction. A cheap OU-with-drift fit on the pooled four-cycle generic-ballot series (also §4) found no statistically significant drift at the live decision's horizon. This is reported as this paper's actual result to date, per §8.1's advance commitment to publish regardless of outcome and to correct rather than quietly patch a finding once a gap in it is found. Section 6 (idiosyncratic uncertainty) remains a proxy treatment, not a fitted process, per §6.2's data-density argument.*
