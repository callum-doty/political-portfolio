...
**Corrected 2026-07-23 (codebase audit):** `data/raw/cook_pvi/` is currently an **empty directory** — neither file above has ever been supplied. Every "Cook Rating" / "Cook-implied win probability" figure currently in FINDINGS.md and the papers is therefore produced entirely by `derive_rating()`'s synthetic PVI+incumbency fallback described below, not by real Cook Political Report data. This should be treated as the operative data source until the proprietary files are actually acquired, and any claim of validating against "Cook's" ratings should be understood as validating against this synthetic proxy.
...
### 2.7 Candidate Periodic Financial Reports — Dated (Paper III, new)

| Attribute | Value |
|-----------|-------|
| **Path** | `data/raw/fec/candidate_periodic_reports_{cycle}.csv` |
| **Cycles** | 2012–2024 (panel), 2026 (live) |
| **Generator** | `fetch_candidate_periodic_reports()` in `scripts/fetch_data.py` — FEC API `/committee/{committee_id}/reports/`, one committee at a time via the candidate-committee crosswalk (`_load_candidate_committee_crosswalk()`, built from `data/raw/candidate_committee_linkage/ccl*.txt`) |

**Columns:** `district_id`, `party`, `cycle`, `fec_candidate_id`, `committee_id`, `coverage_start_date`, `coverage_end_date`, `receipts_period`, `disbursements_period`, `cash_on_hand_end_period`, `report_type_full`, `beginning_image_number`

**Distinct from §2.1 (`candidate_disbursements_{cycle}.csv`):** §2.1 is a single cycle-cumulative-final total per candidate (`TTL_DISB`, from the bulk `weball` file). This is a genuinely **dated, per-filing-period** panel (quarterly, pre-general, post-general, year-end) — one row per Form 3 report a candidate's principal campaign committee actually filed, each with its own coverage window and period-specific receipts/disbursements/cash-on-hand. `docs/theta_followup_plan.md` §0.1.1 previously documented "no per-filing date field anywhere in this repository" for candidate spending; that was true of the bulk files this project used, not of this API endpoint, which nobody had checked until this session (confirmed directly against live FEC API data, not assumed from documentation).

**Amendment handling:** the raw fetch does not resolve amendments — a filing later amended can appear more than once with the identical `(committee_id, coverage_start_date, coverage_end_date)`. `src.backtest.data.fec.load_candidate_periodic_reports()` resolves this at load time by keeping the row with the largest `beginning_image_number` per group (FEC image numbers are date-prefixed and increase monotonically with filing time) — an approximation, not an exact `file_num`/`prev_file_num` chain resolution like §1.4's IE amendment handling.

**Derived point-in-time functions** (`src/backtest/data/fec.py`, mirroring the dated-IE pattern in §1.4/§2.6): `cumulative_candidate_spend_as_of()`, `cumulative_candidate_receipts_as_of()`, `cash_on_hand_as_of()`, `spend_velocity()`, `receipts_velocity()`.

**Pipeline role:** Feeds `dynamic/simulate.py`'s historical point-in-time reconstruction (replacing the previously-fixed candidate-spend floor), `dynamic/state.py`'s `RaceState.cash_on_hand_d` (previously an unpopulated stub), and `scripts/solve_bellman_lsm.py`'s wait-branch spending trickle (`scripts/estimate_candidate_spend_trickle.py`).

**Acquisition note:** requires a registered FEC API key (1,000 req/hr) — the fetch makes one paginated call per House *nominee* (top spender per party per district, `fec.load_candidate_disbursements()`'s roster — ~815–875 per cycle, not every primary candidate in the raw bulk file, which would silently double-count district spend; see `fetch_candidate_periodic_reports`'s docstring), which exhausts DEMO_KEY's 40 req/hr almost immediately. Run via `python scripts/fetch_data.py --only fec-periodic --cycles {cycle} --fec-api-key YOUR_KEY`.

**Fetch reliability, confirmed in practice across all 7 historical cycles plus the live 2026 cycle (2026-07-20/22):** a single cycle's fetch takes 45–90 minutes at this endpoint's pace and does not reliably survive one uninterrupted run in every environment. The fetch is checkpointed — results are appended to a `.partial.csv` after every committee, not accumulated in memory and written once at the end — so an interrupted run can simply be re-invoked with the same command; already-fetched committees are recognized and skipped, and the run resumes rather than restarting. All 7 historical cycles (2012–2024) were fetched this way, several requiring 2–3 restarts each, with zero re-fetched committees and zero lost progress. The 2026 cycle (868 nominees) completed in a single uninterrupted run.
...
### 3.6 GB Dynamics (σ_G, λ) — Paper III §5.3/§6.2

| Attribute | Value |
|-----------|-------|
| **Path** | `data/processed/gb_dynamics.json` |
| **Format** | JSON, flat key-value |
| **Generator** | `scripts/estimate_gb_volatility.py` (`main()`, calling `representative_sigma_g_per_sqrt_day()` and `fit_lambda_from_term_structure()`) |

**Fields:**

| Key | Value | Description |
|-----|-------|--------------|
| `sigma_g_per_sqrt_day` | 0.1838 | Generic-ballot volatility per √day, interpolated at `reference_horizon_days` from the pooled 4-cycle historical term structure |
| `reference_horizon_days` | 120.0 | Interpolation point (matches Paper II §7.1's live-run horizon) — not one of the table's raw rows |
| `lambda_decay` | 0.00536 | Idiosyncratic-uncertainty decay rate (day⁻¹), Section 6.2's `σ_i,t` proxy |
| `tau_days` | 186.5 | `1/lambda_decay` |
| `lambda_fit_A` | 9.74 | Asymptotic variance from the `Var(ΔG)(t)=A(1−e^{−t/τ})` curve fit |

**Single source of truth** (Paper III audit, 2026-07-16): every script that needs σ_G or λ (`simulate_and_validate.py`, `solve_bellman_lsm.py`, `make_theta_paper_figures.py`) reads this file instead of carrying its own hardcoded literal. `scripts/validate_state_simulator.py` re-fits λ from `outputs/gb_volatility_term_structure_historical_only.csv` independently and checks it against this file's stored value (`check_lambda_consistency()`) rather than silently discarding the re-fit.

---

### 3.7 Live 2026 State — Paper II §7.1 / Paper III §8

| Attribute | Value |
|-----------|-------|
| **Path** | `data/processed/live_2026_state.json` |
| **Format** | JSON, flat key-value |
| **Generator** | `scripts/plot_2026_live_allocation.py` (`main()`), re-written every run |

**Fields:**

| Key | Description |
|-----|-------------|
| `as_of` | UTC date the live pipeline was last run (`datetime.now(timezone.utc).date()`) |
| `election_day` | 2026 House general election date, from `config.yaml`'s `dynamic.election_day_2026` |
| `days_remaining` | `election_day − as_of`, in days |
| `budget_2026` | `Bₜ`, from `backtest.model.budget.estimate_budget_2026()` |
| `l_t_committed` | `Lₜ`, real disbursed spend to date (`RealizedSpendCommitmentSource`) |
| `f0` | `Fₜ = budget_2026 − l_t_committed`, the deployable reserve |
| `generic_ballot` | Live 2026 GB point estimate, same value as `config.yaml`'s `generic_ballot_by_cycle.2026` |

**Single source of truth** (Paper III audit, 2026-07-16): `scripts/solve_bellman_lsm.py` and `scripts/estimate_gb_ou_drift.py` read `today`/`election_day`/`f0` from this file instead of independently hardcoded literals — closing a gap that had already produced one real bug (two different "days remaining" figures live in the repo simultaneously, 98 vs. 110, before the fix).

---

### 3.8 Candidate Spend Trickle Rate & Forecast-Floor Seat-Gain Re-estimate — Paper II §7.2 / Paper III §12

| Attribute | Value |
|-----------|-------|
| **Path** | `data/processed/candidate_spend_trickle.json` |
| **Format** | JSON, flat key-value |
| **Generator** | `scripts/estimate_candidate_spend_trickle.py` |

**Fields:** `cycles_used` (list of historical cycles the dated panel §2.7 covers), `by_tier` (per Cook tier: `n_obs`, `median_rate_per_day`, `mean_rate_per_day`, `std_rate_per_day`), `preferred_estimator` (`"mean_rate_per_day"` — median is exactly $0.00/day in every tier, a structural artifact of comparing FEC's quarterly filing cadence against this project's biweekly period grid, not evidence of no growth; see the script's module docstring).

**Pipeline role:** the D-side rate feeds `scripts/solve_bellman_lsm.py`'s wait-branch trickle (`docs/theta_followup_plan.md` §12.3) and `scripts/solve_bellman_lsm_continuous_phi.py`'s analog (§12.6). A per-party generalization of the same calibration (computed inline, not persisted to this file) feeds `scripts/solve_2026_optimal_forecast_floors.py` below.

**Related output** (not a data file per se, but the direct downstream use of this calibration for a static, non-Θ question): `outputs/optimal_2026_forecast_floors.json`, from `scripts/solve_2026_optimal_forecast_floors.py` — re-solves the live 2026 nonlinear optimizer (Paper II §7.1) using BOTH parties' candidate-committee floors projected forward to Election Day at this section's calibrated rate, instead of today's near-zero ones. **Corrected 2026-07-24 (persuasion-ceiling fix, Paper I §7.3b).** Motivation: Paper II §7.1 already named "near-zero candidate-committee floor spending" as part of the mechanism behind that section's non-competitive-seats finding (65.5% pre-audit → 76.8% under 2026-07-23c → **51.4%** under the ceiling fix); the naive live "seat gain from optimal deployment" computed directly from today's floors was inflated by exactly that mechanism (`scripts/investigate_msg_low_d_extrapolation.py`'s already-documented finding that the spending elasticity is disproportionately identified from Safe-tier data), giving +27.4 expected seats pre-ceiling. **The persuasion ceiling caps that same mechanism directly: the immediate-floor estimate is now +9.23 expected seats, and — for the first time — this now matches the forecast-floor re-estimate almost exactly (also +9.23, up from a pre-ceiling +15.75/+21.49 across two prior passes)**, since forecasting floors forward and capping the near-zero-floor gradient both address the identical underlying pathology. **Paper I's validated backtest finding (+2.83 expected seats vs. DCCC's actual 2024 allocation, +3.22 in the 2022 OOS test, both corrected 2026-07-24 — was +13.24/+12.61 under 2026-07-23b, +6.39/+7.77 after the elections.py fix alone, +5.34/+6.79 before that) remains the number to trust for "how much does optimal allocation buy," though the 2026 live estimate (+9.23) is no longer wildly inconsistent with it the way the pre-ceiling figures were — both answer the different, more extreme question of "optimal vs. deploying nothing further," not "optimal vs. what a real committee actually chose." Full discussion: `docs/paper2_draft.md` §7.2.
...
### 4.4 Live Polling — Generic Ballot and District-Level House Races

| Attribute | Value |
|-----------|-------|
| **Paths** | `data/live/generic_ballot_polls.csv`, `data/live/generic_ballot_summary.json`, `data/live/house_district_polls.csv`, `data/live/house_district_polls_summary.json`, `data/live/polling_fetch_log.jsonl` |
| **Generator** | `scripts/fetch_polling.py` — VoteHub's free public API (`https://api.votehub.com`, no API key), same source for both poll types |

**Generic ballot** (`fetch_generic_ballot_polls`, `poll_type=generic-ballot`): individual national poll results plus a simple trailing-average summary (`trailing_average_summary`).

**District-level House races** (new this session — `fetch_house_district_polls`, `poll_type=us-representative`): confirmed live this session that VoteHub carries real per-district subjects (e.g. `"2026 PA-07"`), not just national polls. Columns: `district_id`, `pollster`, `start_date`, `end_date`, `sample_size`, `leader_pct`, `second_pct`, `margin` (leading-candidate-minus-second, **not party-signed** — VoteHub's `answers` list is ordered by candidate, not party), `url`. `house_district_poll_summary()` rolls this up per district as of a given date: `poll_n`, `poll_mean`, `poll_sigma` (`None` if only one poll), `poll_trend` (linear slope of margin over time; `None` if fewer than 2 polls or if all available polls share the same `end_date`, which leaves nothing for the trend fit to identify).

**Coverage, checked directly against the live feed (2026-07-20):** 28 districts had any polling at all, out of the full competitive universe — confirming this is sparse, consistent with `paper3_draft.md` §6.2's prior finding about district-level polling density in general. **No historical (2022/2024) district-level panel exists at this endpoint** — checked directly; VoteHub's `us-representative` coverage is live-2026-only. This means district polling is usable as a **live monitoring/diagnostic signal** (feeding `dynamic/state.py`'s `RaceState.poll_mean_t`/`poll_sigma_t`/`poll_n_t`/`poll_trend_t`) but **not** as a dataset for fitting a historical `ε_{i,t}` process — §6.2's conclusion about the infeasibility of estimating district-level idiosyncratic uncertainty from data stands unchanged.

**Same caveat as the generic-ballot series** (`fetch_polling.py`'s module docstring): diagnostic only, not wired into `RaceState.mu_hat` or any structural model input. There is no historical panel to validate a within-cycle coefficient against — feeding a live poll average into the margin model would repeat the exact mistake already flagged for generic-ballot polls (§1.8b's `α₃` scope boundary).

---

### 4.5 2026 Live Allocator Comparison — Paper I / Paper II §7.2

| Attribute | Value |
|-----------|-------|
| **Paths** | `outputs/allocator_comparison_2026_live_competitive.png`/`.csv`, `outputs/allocator_comparison_2026_live_all.png`/`.csv` |
| **Generator** | `scripts/plot_allocator_comparison_2026.py` |

The live, in-progress analog of `outputs/allocator_comparison.png` (Paper I's completed-2024-cycle four-way efficiency comparison, §2.1/§2.6/§3.1) — the same "where the money is vs. where the model says it should be" comparison, applied to a decision that hasn't been made yet rather than one already decided.

**Series plotted, per race:** actual spend to date (candidate-committee filings, §2.7, plus coordinated/IE, `RealizedSpendCommitmentSource`); a trend forecast (candidate-committee component projected to Election Day at §3.8's calibrated trickle rate, coordinated/IE held flat — labeled on the chart as a likely underestimate, for the reason given in §3.8); the model's recommended total spend by Election Day (`plot_2026_live_allocation.py`'s `run_live_allocation()`, unmodified); Cook-implied and Uniform benchmarks, scaled to the actual-spend-to-date total.

**`competitive` scope** (55 races: Toss-Up/Lean D/Lean R): per-race x-axis labels, matching the 2024 chart's layout. **`all` scope** (434 races): no outcome rings possible (election hasn't happened) — replaced with continuous shading by the model's current `P(D win)` estimate, explicitly captioned as an estimate, not a result; per-race labels dropped (illegible at this count) with a handful of extreme races annotated instead.

**A real, checked finding, not a hypothetical:** the `all` scope's total actual-to-date spend ($547M) exceeds the model's total target ($394M, the DCCC's own controllable budget) — this is not an efficiency signal, it's two different pools of money. The $547M includes every candidate's own committee spending nationwide (most of it self-funded money in races DCCC has no strategic reason to touch); the largest single-district totals in the entire dataset (MD-06, IL-08, NY-14, NY-08 — $15–27M each) are all in non-competitive seats. MD-06's figure was checked directly against the raw filing data rather than assumed to be a data error: it is David Trone, the self-funding businessman who spent over $60M of his own money in a 2024 Senate primary, doing something similar in this House race — the model correctly assigns that race $0 in additional DCCC money, itself a clean illustration of the marginal-dollar logic this whole project is built on.
...
| `dynamic.period_days` | 14 | Biweekly grid spacing; single source of truth for `scripts/simulate_and_validate.py` and `scripts/solve_bellman_lsm.py` (`backtest.config.period_days()`), was 3 independent hardcoded literals before the Paper III audit (2026-07-16) |
| `dynamic.election_day_2026` | `"2026-11-03"` | 2026 House general election date (`backtest.config.election_day(2026)`); read into `data/processed/live_2026_state.json` |
| `budget_2026_projection` | See §3.7 / `backtest.model.budget.estimate_budget_2026()` | BLS CPI-U inputs (raw index values, genuine external data this repo does not fetch) used to inflate the 2018/2022 party-controlled budgets to a 2026-equivalent `Bₜ` — replaces a hardcoded `$394.3M` literal whose derivation previously existed only as prose |
