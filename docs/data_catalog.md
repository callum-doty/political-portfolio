...
**Related output** (not a data file per se, but the direct downstream use of this calibration for a static, non-Θ question): `outputs/optimal_2026_forecast_floors.json`, from `scripts/solve_2026_optimal_forecast_floors.py` — re-solves the live 2026 nonlinear optimizer (Paper II §7.1) using BOTH parties' candidate-committee floors projected forward to Election Day at this section's calibrated rate, instead of today's near-zero ones. Motivation: Paper II §7.1 already named "near-zero candidate-committee floor spending" as part of the mechanism behind that section's 65.5%-to-non-competitive-seats finding; the naive live "seat gain from optimal deployment" computed directly from today's floors (+27.4 expected seats) is inflated by exactly that mechanism (`scripts/investigate_msg_low_d_extrapolation.py`'s already-documented finding that the spending elasticity is disproportionately identified from Safe-tier data). The forecast-floor re-estimate reduces this to +15.75 seats — still likely an overestimate, since coordinated/IE spend is held flat rather than projected (that component is the most extreme back-loader of all, per `docs/theta_followup_plan.md` §10) and even the candidate-side forecast uses a smooth average against spending known to be lumpy. **Paper I's validated backtest finding (+5.34 expected seats vs. DCCC's actual 2024 allocation, +6.79 in the 2022 OOS test) remains the number to trust for "how much does optimal allocation buy" — both 2026 live figures answer the different, more extreme question of "optimal vs. deploying nothing further," not "optimal vs. what a real committee actually chose."** (Corrected 2026-07-22: this entry previously cited +4.46/+5.36, an interim pre-α₅-fix figure per FINDINGS.md §10.3, not Paper I's final validated result.) Full discussion: `docs/paper2_draft.md` §7.2.
