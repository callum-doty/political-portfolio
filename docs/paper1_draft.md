...
A second, more direct test addresses a distinct concern: that the model optimizer's seat gain over DCCC (Section 10) reflects mainly the concavity of the win-probability curve — i.e., that almost any reallocation of the same dollars would look like an improvement — rather than genuine MSG-based targeting. `permutation_test_allocation_efficiency()` (`comparison/benchmark.py`) randomly reshuffles DCCC's own per-race dollar amounts across the same 53 races (same multiset of dollars, no relationship to MSG) and evaluates E[Seats] under each shuffle using the same linearized approximation applied to the Null and Cook-implied benchmark rows in Table 3. Two results, both from 2000 shuffles:

- DCCC's actual allocation (E[Seats] = 215.18) is matched or exceeded by 100% of random reshuffles of its own dollars (null mean 218.29, 95% CI [216.89, 219.62]). DCCC's specific choice of which race receives which dollar amount is not merely rank-correlated with low MSG — no random reassignment of the same money scored worse.
- The model optimizer's linearized allocation (E[Seats] = 224.91; the nonlinear SLSQP figure of 220.52 in Table 3 remains the authoritative headline number, per the note below that table) is matched or exceeded by 0 of 2000 reshuffles. The optimizer's advantage is not attributable to reallocation-in-general; it depends on targeting specifically.

Both tests use seed 42 and `n_permutations=2000` (`config.yaml: uncertainty.permutation_draws`), run automatically in `scripts/run_backtest.py` and saved to `outputs/permutation_tests.json`.
