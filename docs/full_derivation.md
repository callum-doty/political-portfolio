...
**Not** Part I §I.6's structural $\beta_i=\phi(\mu_i/\sigma_i)\alpha_3/\sigma_i$. `estimation/factors.py` fits a 5-factor ridge regression — national generic ballot, 3 Census-region dummies × GB, urbanicity share × GB — via `RidgeCV` on the 2012–2022 panel, intending $\text{Cov}=\text{loadings}\times\text{factor\_cov}\times\text{loadings}^T$.

**Corrected 2026-07-23 (codebase audit):** this module is *not* what the pipeline actually runs. `build_factor_matrix()` has zero call sites in `run_backtest.py`, `run_dynamic_backtest.py`, or any live-2026 script — every real caller uses `build_dummy_factor_model()` (defined inline in `scripts/run_backtest.py`), a single-factor generic-ballot-only placeholder. Separately, `build_factor_matrix()` itself has a bug independent of being unwired: it fits `RidgeCV` but never applies `ridge.coef_` to the loadings — `loadings` are just the raw 2024 regressor values (`gb`, `region×gb`, `urban×gb`), not those values scaled by the estimated ridge sensitivities, so `factor_cov` is the covariance of raw covariates rather than of anything the regression estimated. Both issues must be fixed before this module could be considered "estimated" in any meaningful sense. Every Var[Seats]/γ-penalty figure in both papers is currently produced by the single-factor GB-only placeholder, not this 5-factor model.
