...
**A naive "seat gain from optimal deployment" estimate computed from this run is not trustworthy, and the reason is the same near-zero-floor mechanism.** Comparing the model-optimal expected-seats figure (241.12, this section's Table 5 run) against expected seats if the deployable reserve Fₜ is not spent at all gives a raw gain of **+27.4 seats** — implausibly large next to Paper I's validated finding that the model beats DCCC's *actual* 2024 allocation by only +5.34 seats (+6.79 in the 2022 out-of-sample test; corrected 2026-07-22 — an earlier draft of this paragraph cited +4.46/+5.36, an interim pre-α₅-fix figure per FINDINGS.md §10.3, not Paper I's final validated result). The gap is explained by `scripts/investigate_msg_low_d_extrapolation.py`'s earlier finding (referenced but not yet connected to a live number when Section 7.1 was first drafted): the margin model's spending elasticity (`beta1`, identified via repeat-challenger pairs) is disproportionately estimated from Safe-tier low-spend races (85 of 118 pairs), not competitive ones (14 of 118) — applying it to dozens of simultaneously near-zero-floor competitive races at once amplifies that untested assumption rather than averaging across it.

`scripts/solve_2026_optimal_forecast_floors.py` re-solves the same optimizer using floors projected forward to Election Day at the calibrated per-tier candidate-committee trickle rate (Paper III Section 12.7's D-side rate, generalized to both parties here — projecting D forward while leaving R frozen would trade one asymmetry for another) instead of today's near-zero ones. This reduces the raw gain to **+15.75 seats** — a real improvement, but still likely an overestimate, for two identified reasons that were not corrected: outside-group (coordinated/IE) spending is held flat rather than projected forward at all, and this project's own research (`docs/theta_followup_plan.md` Section 10) already found that component to be the most extreme back-loader of all (95%+ arrives in the final two months) — a smooth trend line is a poor model for it specifically. Even the candidate-side forecast uses a smoothed average rate against spending that is known to be lumpy and to accelerate near Election Day, not linear.

**The number that should actually be trusted for "how much does optimal allocation buy" is Paper I's validated +5.34/+6.79-seat backtest finding, not either 2026 live estimate.** Both live figures (+27.4 and +15.75) answer a different, more extreme question than Paper I's backtest does — "optimal vs. deploying nothing further," which no real committee would do — rather than "optimal vs. what a real committee actually chose," which is what Paper I measured against a completed cycle's real DCCC behavior. The live 2026 estimates are directionally consistent (optimal deployment helps) but should not be quoted as more precise than Paper I's validated range; the gap between the two 2026 numbers is itself the clearest evidence of how sensitive this specific live computation still is to floor maturity.
