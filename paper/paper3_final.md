...
## Main Result Under the LP Control Approximation -- Status Suspended (See Section 8.9)

**The figures in this section, and Tables 11--13 throughout this paper, are estimates under the LP within-period control approximation, not yet demonstrated properties of this paper's intended nonlinear allocation model.** Section 8.9 reports a preliminary allocator-robustness check finding these figures do not survive replacing the LP allocator with the true nonlinear allocator throughout the backward induction, and suspends the practical "hold the reserve" recommendation pending further investigation. The material below is retained because it remains a valid, internally-consistent description of what the LP-based calculation produces, which is itself informative (Section 8.9 argues the LP's own discontinuous, knapsack-like behavior may be manufacturing apparent option value) -- but it should not be read as this paper's settled empirical conclusion.
...
**Table 11: $\Theta(0)$ at the Live 98-Day Horizon (LP Control Approximation)**
...
$\Theta_{\text{LP}}(0)$ is substantially positive in every scenario, and holding the deployable reserve is the unanimous recommendation *under the LP allocator*. This is the result of a calculation that has now been through three rounds of correction: an earlier, incorrect version found the opposite sign (missing the candidate-spending trickle channel entirely); the version after that found a positive but much smaller $\Theta(0)$ ($+1.3$ to $+1.7$) due to a variance-specification double-count in the deploy-branch convolution; and external review of that fix caught a further, related inconsistency at the $t=T$ boundary specifically, corrected in Section 4.5 and reported in full in Section 8.6. This last correction moved the figures modestly (roughly $\pm0.4$ seats per scenario) rather than by the multiple-fold swing the earlier double-count fix produced. A fourth, more serious issue supersedes these: Section 8.9 shows the LP-vs-nonlinear allocator choice itself moves $\Theta(0)$ by more than any of the three corrections above, in a direction that reverses its sign.
...
![$\Theta(t)$'s full schedule across the live 98-day horizon, all three scenarios.](figures/theta_schedule_over_time_fig.png){width=95%}
...
**Table 12: $\Theta_{\text{LP}}(0)$ at the 364-Day Counterfactual Horizon (LP Control Approximation -- Suspended, Section 8.9)**
...
**Table 13: Continuous Deployment-Fraction Generalization, 5-Point Grid (LP Control Approximation -- Suspended, Section 8.9)**
...
The corner holds cleanly in the two single-cycle brackets, now by a wider margin than before correction. `eta_bootstrap_all_cycles` no longer places any mass at all on full immediate deployment (0.0%, versus 18.2% under the uncorrected convolution) and instead spreads its non-hold mass across the interior fractions -- a genuinely different qualitative shape than the near-bimodal (0%/100%) pattern the uncorrected model produced, consistent with the corrected model no longer artificially compressing win probabilities toward 0.5 at intermediate budget levels. "Hold" still wins by a wide margin on average. (Note: which single-cycle bracket has the smallest gap has changed across each round of correction in this section -- `eta_fit_2022` at one point, `eta_bootstrap_all_cycles` now -- so this paper no longer singles one out as "closest to indifference"; the 11-point spot-check below covers both single-cycle brackets rather than picking whichever happens to be nearest at a given correction.) An 11-point spot-check confirms the 5-point result under the corrected convolution for both single-cycle brackets: `eta_fit_2024` (the representative case shown in the figure above's panels A--B) gives gap $=+3.762$ seats versus $+3.751$ at 5 points ($0.011$-seat difference); `eta_fit_2022` gives gap $=+3.244$ versus $+3.236$ ($0.008$-seat difference). Both are 100.0% hold at every grid point from 10\% through 90\%, confirming the corner is not a grid-resolution artifact in either case.
...
**Table 13b: Mechanism Decomposition of $\Theta_{\text{LP}}(0)$ (LP Control Approximation -- Suspended, Section 8.9)**
...
Scenario A -- nothing evolves over the horizon at all -- gives exactly $\Theta(0)=0$, the sanity check the decomposition machinery must pass before the other four rows can be trusted; it still does after the terminal-boundary correction described above. Scenarios B and C isolate each channel on its own: **deterministic sequencing value (B, $+3.479$) is larger than pure information value (C, $+1.485$)**, confirming the reviewer's suspicion directly rather than refuting it. Roughly 70% of the two channels' combined single-channel value ($3.479/(3.479+1.485)$) comes from candidate committees' predictable organic spending growth, not from resolving genuine uncertainty -- essentially unchanged from before the terminal-boundary correction, because Scenarios A and B both run with `enable_stochastic=False` and were therefore unaffected by it; only C, D, and E (which use the stochastic path where the boundary inconsistency lived) moved, and none by enough to change this conclusion. Scenario D (both channels, no opponent reaction) exceeds the sum of B and C, indicating the two channels interact super-additively rather than simply adding; adding opponent reaction (E) then reduces $\Theta(0)$ from D's $+5.071$ to the full model's $+4.714$, since reaction dampens the wait branch's advantage by having $R_{i,t}$ partially offset the trickle it drives.

**The honest claim, and the one this decomposition does not license.** Per the reviewer's own proposed resolution: the defensible claim is that this model estimates a positive value of deferring party deployment under *both* predictable candidate-spending growth *and* resolving uncertainty, with the deterministic channel contributing the larger share in isolation. The stronger claim -- that $\Theta$ specifically prices information-driven flexibility -- is not supported by this decomposition; Scenario C shows genuine information value is real and positive ($+1.485$) but is not, on its own, the dominant driver of the headline result.
...
**Monte Carlo standard error.** Re-solving $\Theta(0)$ at $K=2{,}000$ across 5 independent seeds gives a mean of $+4.740$ with sample SD $0.099$ and standard error $0.044$ expected seats -- simulation noise is under 1\% of the point estimate, not a material source of uncertainty relative to the roughly 3--4$\times$ swing Section 8.6's variance-double-count correction produced (the terminal-boundary refinement that followed it moved figures by a few tenths of a seat, a magnitude these same checks below also cover).

**Out-of-sample policy evaluation.** Refitting with 30\% of paths held out of every period's continuation-value regression (so a held-out path's own realized future value never informs the regression that decides its stopping choice) gives $\Theta(0)=+4.599$ on the held-out paths versus $+4.604$ in-sample -- a 0.1\% difference. The compressed, five-feature regression basis (Section 4.6) does not appear to be meaningfully overfitting at this sample size.

**$K$-sensitivity.** $\Theta(0)=+4.644$ at $K=2{,}000$ versus $+4.769$ at $K=5{,}000$ (single seed) -- consistent with the multi-seed SE above, and well within the range needed to trust the headline figures at the precision reported.

None of these checks change the qualitative or quantitative conclusion; they establish that remaining Monte Carlo and regression-specification uncertainty is small relative to $\Theta(0)$'s magnitude, which is not the same claim as small relative to the *calibration and parameter* uncertainty discussed in Limitations below.

![Monte Carlo and regression-specification diagnostics, `eta_bootstrap_all_cycles`: (A) five-seed agreement, (B) K-sensitivity, (C) held-out policy evaluation against in-sample.](figures/theta_convergence_diagnostics_fig.png){width=100%}

Section 8.8's checks establish that Monte Carlo noise and regression specification are small sources of uncertainty *for the LP-based calculation specifically*. They say nothing about whether the LP allocator itself is the right thing to be running this uncertainty analysis on. Section 8.9 shows it is not.

## A Critical, Unresolved Allocator-Robustness Finding
...
**$K=30$ confirms the effect does not wash out as precision increases.** The remaining half of item (2) is an actual increase in $K$, not more replicates at $K=15$ -- more seeds at a fixed, small $K$ cannot substitute for reducing each individual estimate's own noise. A single $K=30$ replicate (double the path count, hence roughly double the compute cost, an independent seed and independent state paths from all $K=15$ runs):

**Table 13g: $K=30$, Paired**

| $K$ | $\Theta_{\text{LP}}(0)$ | $\Theta_{\text{nonlinear}}(0)$ | $\Delta_{\text{allocator}}$ |
|---|---|---|---|
| 15 (mean of 3 seeds) | $+4.463$ | $-0.259$ | $-4.722$ |
| 30 (1 seed) | $+4.268$ | $-0.435$ | $-4.703$ |

Doubling $K$ moved $\Delta_{\text{allocator}}$ by less than half a percent ($-4.722\to-4.703$), and $\Theta_{\text{nonlinear}}(0)$ remained negative and within the range the three $K=15$ seeds already spanned. This is the single most reassuring result in the investigation so far: if the reversal were a small-$K$ artifact that would resolve toward zero or flip positive with more precision, doubling $K$ should have moved it visibly. It did not.

**What this result establishes.** The estimated value of waiting is highly dependent on the within-period allocator, by an amount (roughly 4.3--5.5 seats, consistent across the unpaired comparison, the paired comparison, three $K=15$ seeds, and now one $K=30$ replicate) far too large to be ordinary Monte Carlo variation around the LP result, and stable under a real increase in path count. The LP-based calculation cannot, at present, support statements such as "holding is the unanimous recommendation," "waiting is worth $+4.6$ to $+5.9$ expected seats," "the result is robust across calibration scenarios," or "the optimal policy favors holding the entire reserve." Those are results of the LP approximation, not yet demonstrated properties of the underlying nonlinear model this paper intends to specify. The period-by-period decomposition further establishes a specific, quantified mechanism: the LP allocator's static, state-insensitive concentration in a handful of races penalizes immediate deployment more heavily earlier in the horizon than later, which by itself would inflate the apparent value of waiting.

**What this result does not establish.** No single replicate at $K=15$ or $K=30$ (Tables 13c--13g) individually establishes that immediate deployment is preferable, and four total replicates (three at $K=15$, one at $K=30$) is not enough to replace a proper, larger-$K$ progression with multiple seeds at each level. The continuation regressions remain data-starved at these path counts (five portfolio-level features fit against a tiny effective sample); the stopping rule can change from a handful of paths; nonlinear-solver variation (SLSQP convergence differs across states, sometimes needing far more iterations than others) may add noise the LP runs do not have. A rough scaling of Section 8.8's $K=2{,}000$ LP standard error ($0.044$) by $\sqrt{2{,}000/K}$ suggests an approximate SE near $0.5$ at $K=15$ and $0.35$ at $K=30$ -- broadly consistent with the observed cross-seed SD of $0.109$ on $\Theta_{\text{nonlinear}}(0)$ at $K=15$, though this scaling assumes the same estimator with stable asymptotic behavior, which the nonlinear control's different regression targets and stopping decisions may not fully satisfy. Pairing, the multi-seed check, and the $K=30$ replicate each addressed a different specific source of doubt (simulated-world artifact; lucky/unlucky single draw; small-$K$ instability that should wash out with more precision) and none of them moved the answer; none of this yet substitutes for out-of-sample policy evaluation or multiple seeds at $K=30$ and beyond. The defensible conclusion at this stage is that the nonlinear estimate is consistent with a small negative or near-zero value of waiting, increasingly rather than merely "not positive" -- but still short of a confident sign determination.
...
**What remains to be done before this is resolved.** A structured investigation is required before $\Theta(0)$'s sign under the intended nonlinear model can be reported with confidence. Item (1), paired simulation using common random numbers, **is done** (Table 13d): the reversal survives pairing. Item (2)'s first sub-step, multiple seeds at $K=15$, **is done** (Table 13e): three independent seeds all show the same reversal, tightly clustered. Item (4), a period-by-period mechanism decomposition, **is done** (Table 13f): the mechanism is the LP's state-insensitive concentration penalizing early deployment more than late. Item (2)'s next step, an actual increase in $K$, **has begun** (Table 13g): one $K=30$ replicate shows the same effect at essentially the same magnitude. Remaining: multiple seeds at $K=30$ (matching the rigor already applied at $K=15$, not just the single replicate so far), then $K=50$ and $K=100$ if $K=30$'s pattern holds; (3) out-of-sample policy evaluation (Section 8.8's held-out-path methodology) applied to the nonlinear-throughout run once $K$ is large enough to support it; (5) a validated fast surrogate that preserves diminishing returns (a piecewise-linear concave approximation, a precomputed per-race value-on-a-spending-grid table, or a model trained on nonlinear-optimizer outputs) validated against the true nonlinear solver before being trusted for a full $K=2{,}000$-scale re-solve; and (6) only after the `eta_bootstrap_all_cycles` scenario stabilizes, repeating across `eta_fit_2022`, `eta_fit_2024`, and the continuous deployment-fraction framing -- the 364-day comparative-static extension is lower priority still. This section now reports a substantially more complete case that the reversal is real and stable under increasing precision, plus its mechanism -- but not yet a large-enough-$K$ resolution of $\Theta(0)$'s sign under the intended nonlinear model, and the paper's hold recommendation remains suspended.

\newpage

# Discussion

## Why the Corner Flipped (Within the LP Approximation -- Itself Now Suspended)

**This subsection describes the LP-based calculation's own internal history and should be read alongside Section 8.9's finding that the LP figures it explains are not currently trustworthy as a policy conclusion.** The mechanism is structural, not a recalibration accident. Once candidate-committee spending is allowed to grow deterministically while a committee waits, and opponent reaction is correctly credited with responding to that growth, the wait branch's simulated future states genuinely differ from -- and, on average, exceed -- what the deploy branch's convolution identity captures for a static $D_{i,t}$. The earlier, "deploy favored" result was not wrong because the option-value logic of Papers II--III is wrong; it was wrong because the model actually tested was missing a real channel through which waiting pays off. This is a direct illustration of Section 3.3's methodological claim: a Bellman equation over an incompletely specified $P$ does not merely give an imprecise answer, it can give the *wrong-signed* one. The subsequent variance-double-count correction (Sections 4.5, 8.6) is a second illustration of the same claim, one level down: even with the transition law's *components* correctly specified, an error in how two of their variance terms combine changed $\Theta_{\text{LP}}(0)$'s *magnitude* by a factor of roughly 3--4, without changing its sign. Section 8.9's allocator-robustness finding is a third, more consequential illustration still: a specification error in the *control* (the within-period allocator), not the transition law $P$ this paper is nominally about, appears large enough to change $\Theta(0)$'s sign a second time, and this one is not yet resolved.
...
Paper II's own live run, using the $\Theta$-free rollout architecture, recommends deploying the full reserve immediately, concentrated disproportionately in non-competitive seats. The LP-based version of this paper's result recommended the opposite action at the same decision point, and the two were framed as exactly the comparison this research program is built to make: Paper II identifies and measures the size of a gap; this paper specifies the process that determines which side of that gap the live decision actually falls on. Section 8.9 complicates this reconciliation directly -- with the nonlinear allocator, this paper's own preliminary estimate moves back toward Paper II's "deploy" recommendation, at least at the reduced precision so far available. Which side of the gap the live decision actually falls on is, at present, unresolved by either paper, not settled in Paper III's favor.

## A Domain-Intuition Check Against Real Data

A model recommendation this consequential should not be accepted on the strength of internal consistency alone. Real independent-expenditure committees spent only 1.6--4.4% of their eventual full-cycle total by September 1, in both the 2022 and 2024 cycles and both parties -- near-total real-world back-loading, and strong independent evidence that sophisticated political spenders behave nothing like an immediate-deployment strategy, for reasons this model may or may not fully capture (genuine information value, strategic waiting, targeting immaturity early in a cycle, or some mix). The LP-based result reported in Section 8.3--8.5 is consistent with this pattern; the earlier, uncorrected LP-based result directly contradicted it. This corroborating evidence should be read with the same caution Section 8.9 now attaches to the LP-based figures themselves: real committees' backloading is independent evidence worth weighing, but it cannot substitute for resolving which allocator's $\Theta(0)$ estimate is actually correct, and it is not, on its own, a reason to prefer the LP-based conclusion over the nonlinear one.
...
## Allocator Robustness (Critical, Unresolved -- See Section 8.9)

Every limitation below describes the LP-based calculation. That calculation's own headline conclusion is currently suspended: Section 8.9 finds that replacing the LP within-period allocator with the true nonlinear allocator throughout the backward induction reverses $\Theta(0)$'s sign at a reduced path count, by a margin (roughly 5.5 seats) far exceeding this paper's own Monte Carlo noise estimate. This is listed first, not folded into "Computational" below, because it is not a precision issue -- it is an open question about which sign is correct, and it takes priority over every other limitation in this section until resolved.
...
**No practical recommendation is currently made.** An earlier version of this section stated that the LP-based finding -- hold the reserve at the live 2026 decision -- was a recommendation about a single, specific decision point, re-derived from the current state each time the pipeline is re-run. Section 8.9 supersedes that framing: until the allocator-robustness question is resolved, this paper does not recommend either holding or deploying the live 2026 reserve. Paper II's own architecture (Section 4.7 of that paper) already anticipates that a $\Theta$-aware recommendation must be recomputed as the state evolves, not applied once and left unrevisited; that design is unaffected by the present suspension, but there is presently no $\Theta$-aware recommendation to feed into it. This paper's Longstaff--Schwartz solve has not been folded back into Paper II's rollout loop as a per-period, automatically-applied reserve fraction, and should not be until Section 8.9's investigation concludes.
...
Having specified and estimated $P$, solved the resulting Bellman equation, and corrected real errors found by checking mechanism before trusting output -- a missing spending-drift channel, an asymmetric convolution, and a variance double-count found in external review and resolved in two passes as that review's own scrutiny deepened -- this paper's LP-based calculation found $\Theta_{\text{LP}}(0)$ substantially positive ($+4.5$ to $+5.1$ expected seats) in every calibration scenario tested, in both a binary and a continuous deployment-fraction framing, at both a 98-day horizon and a 364-day comparative-static extension, with holding the deployable reserve dominating deploying it. **That finding is no longer this paper's conclusion.** Section 8.9 reports a further discovery, made by turning the same mechanism-checking discipline on the control itself rather than only on the transition law $P$: replacing the LP within-period allocator with the true, diminishing-returns-respecting nonlinear allocator throughout the backward induction reverses $\Theta(0)$'s sign at a reduced path count, by a margin far exceeding this paper's own quantified Monte Carlo noise. A mechanism decomposition (Section 8.7) had already shown that a majority of the LP-based $\Theta$'s magnitude, isolated channel by channel, traces to predictable candidate-spending growth rather than classical information-option value -- itself a real, quantified qualification of the paper's original framing -- but Section 8.9's finding is more fundamental: it is not yet known which sign the correctly specified nonlinear model implies, and this paper does not currently endorse either "hold" or "deploy" as its live 2026 recommendation.

This is not a failure of the paper's central methodological claim; it is a further instance of it. Section 3.3 argues that an optimal-stopping calculation is not well-posed until its underlying transition law is specified -- that specifying $P$ is the actual scientific content of the problem, not the machinery that follows from it. Section 8.9 shows the same discipline extends to the *control*: an optimal-stopping calculation is equally not trustworthy until the within-period optimization it depends on is verified to be the one actually intended, not a tractable substitute assumed innocuous. This paper specified and estimated $P$ successfully. It has not yet verified that its reported $\Theta$ is a property of the intended nonlinear control rather than an artifact of the LP control's own greedy, discontinuous behavior. Together, the three papers in this research program price the next dollar (Paper I), build the system that deploys it repeatedly (Paper II), and attempt to price the alternative of not yet deploying it (this paper) -- and, at the specific decision this program was built to inform, this paper currently has no defensible answer to give, pending the investigation Section 8.9 lays out.
...
## B.1 The Deploy-Branch Terminal-Probability Identity (Corrected)
...
- [ ] **CRITICAL, UNRESOLVED:** the LP-vs-nonlinear allocator choice reverses $\Theta(0)$'s sign at a reduced path count ($K=15$); the sign under the correctly specified nonlinear control is not yet established (Section 8.9). No claim in Tables 11--13b should be cited as this paper's settled conclusion until this is resolved.
