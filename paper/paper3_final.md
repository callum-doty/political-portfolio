...
**Extending the paired classification-agreement check beyond the bootstrap scenario.** Every exact nonlinear-throughout run reported so far (the $K=15$--$100$ progression above) used `eta_bootstrap_all_cycles` exclusively -- a real gap, since a reviewer could reasonably ask whether the surrogate remains trustworthy for the two single-cycle brackets specifically. This is checked directly with the same paired, common-random-numbers methodology (identical simulated $d_{i,t}$, $r_{i,t}$, $\mu_{i,t}$, $\varepsilon_i^{\text{cum}}(t)$ paths for both allocators), now also reporting per-path agreement in the deploy-vs-wait classification at $t=0$, not just agreement of the mean:

**Table 13o: Surrogate-Throughout vs. Nonlinear-Throughout, Paired, $K=15$, Single-Cycle Brackets**

| Scenario | $\Theta_{\text{surrogate}}(0)$ | $\Theta_{\text{nonlinear}}(0)$ | Per-path classification agreement | Nonlinear-throughout wall time |
|---|---|---|---|---|
| `eta_fit_2022` | $-0.317$ | $-0.383$ | $\mathbf{100\%}$ (15/15 paths) | $9.0$ hours |
| `eta_fit_2024` | $-0.453$ | $-0.461$ | $\mathbf{100\%}$ (15/15 paths) | $9.7$ hours |

For both single-cycle brackets, the surrogate and the true nonlinear solver do not merely agree in aggregate mean $\Theta(0)$ (within $0.066$ and $0.008$ seats respectively, the same order of magnitude as, or tighter than, the single-state validation gap reported above) -- every one of the 15 simulated paths in each scenario reaches the identical deploy-vs-wait decision under both allocators. This is the strongest form of agreement this paper checks for: not a close aggregate number that could in principle average over path-level disagreements in opposite directions, but unanimous path-by-path concurrence, now established for all three calibration scenarios (the bootstrap scenario's own $K=15$--$100$ progression, Tables 13e/13g, already showed this at the aggregate level; this extends it to the two single-cycle brackets specifically, and to per-path agreement rather than only the mean).
