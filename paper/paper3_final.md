...
**What remains to be done before this is resolved.** Item (1), paired simulation using common random numbers, **is done** (Table 13d): the reversal survives pairing. Item (2), the incremental-$K$ progression with multiple seeds, **is substantially done through $K=30$ and extended (single-seed) through $K=100$** (Table 13g): the estimate is converging, not diverging or flipping. Item (3), out-of-sample policy evaluation, **is done** (Table 13h): no overfitting artifact is found. Item (4), a period-by-period mechanism decomposition, **is done** (Table 13f). Remaining: multiple seeds at $K=50$ and $K=100$ to formalize the confidence the single-seed extensions only suggest; (5) a validated fast surrogate that preserves diminishing returns (a piecewise-linear concave approximation, a precomputed per-race value-on-a-spending-grid table, or a model trained on nonlinear-optimizer outputs) validated against the true nonlinear solver before being trusted for a full $K=2{,}000$-scale re-solve -- likely the only realistic path to that headline-precision figure, given the true nonlinear solver's per-call cost; and (6) only after the `eta_bootstrap_all_cycles` scenario is fully confirmed, repeating across `eta_fit_2022`, `eta_fit_2024`, and the continuous deployment-fraction framing -- the 364-day comparative-static extension is lower priority still. This section now reports a substantial and increasingly hard-to-dismiss case that the true value of waiting under the intended nonlinear model is small and negative, with a specific, quantified mechanism explaining why the LP approximation reported the opposite -- but formal statistical confidence at high $K$, and confirmation across the other two calibration scenarios, remain open, and the paper's hold recommendation remains suspended rather than reversed outright.
