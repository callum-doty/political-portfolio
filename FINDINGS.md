...
**Corrected 2026-07-27 (floor-margin convention unification, no numeric impact).** `model/win_prob.py` and `optimizer/allocator.py` had independently re-derived the floor-ratio computation feeding `mu_floor` above — `win_prob.py` clipped the *ratio* to `[1e-6, 1-1e-6]`, `allocator.py` floored the *dollar amounts* to $1 before forming the ratio. These disagree by up to ~23pp of `mu_floor` for the 18 2024 races with a literal $0 candidate-committee floor (e.g. LA-04), but the disagreement was inert: every such race is lopsided enough that Φ₀ saturates to ~0 under either convention, so `C_i` itself was identical to three decimal places in every case (checked directly, not assumed). Both callers now share one implementation, `model.margin.predict_floor_margin()`, using the dollar-floor convention — the statistically faithful one, since it preserves log(D/(D+R))'s relative-spending information as D→0, where ratio-clipping instead collapses every sufficiently lopsided race to the same value regardless of the opponent's actual spend. Separately, `allocator.py`'s vectorized `mu_const` was found to omit the α₅·indiv_share term `margin.predict()` includes — also fixed, also inert today since α₅=0.0 (§5.4). Neither fix changes any number in this document; both close a latent divergence risk for whenever α₅ is estimated non-zero (§10.7 Gap 1) or a future data refresh moves one of the 18 zero-floor races off the saturation region. Full derivation: `docs/full_derivation.md` §III.22.
...
### 9.4b Adversarial Response as a Genuine Nash Equilibrium (closes §10.5's "left as future work")

§9.4's η model is a one-sided, mechanical reaction rule: R reacts to D, but D's own allocation is never solved jointly against a strategic R. This section replaces it with NRCC modeled as its own constrained optimizer — maximizing its own expected seats, subject to its own budget, using the *same* margin model and win-probability formula DCCC's optimizer uses — and solves the joint fixed point of both committees' simultaneous best responses via best-response dynamics (Gauss-Seidel fictitious play: each round, D re-solves fully against R's latest allocation, then R re-solves against D's just-updated allocation; neither side is a leader). Implementation: `src/backtest/optimizer/nash.py`; runner: `scripts/solve_nash_equilibrium.py`; tests: `tests/test_nash.py` (9 tests, all passing).

**A real, flagged asymmetry.** D's best response reuses `optimize_nonlinear()` unmodified, including the persuasion ceiling (§7.3b), which was empirically calibrated against DCCC's own historical spending-response data. R's best response uses a mirrored ceiling construction — same functional form, anchored at R's own candidate-committee floor instead of D's, reusing the same `c_max` — but this mirror is **not** independently calibrated against NRCC-specific data (none exists in this project). It exists to prevent the same μ-extrapolation blowup on R's side that the original ceiling exists to prevent on D's (model/ceiling.py's own stated purpose), not because an NRCC-specific `c_max` has been validated. NRCC's own candidate-committee floor (`cand_r_total`) is not a `RaceRecord` field — loaded directly from `fec.load_candidate_disbursements(cycle)` filtered to `party="R"`, kept as an explicit parameter throughout rather than added to the shared type, so this extension touches nothing else in the pipeline.

**2024 result** (`python scripts/solve_nash_equilibrium.py --cycle 2024 --damping-theta 0.5 --max-rounds 40`, real fitted coefficients, full 433-race universe, ~50 min wall clock):

| | E[seats] |
|---|---|
| DCCC observed baseline (§9.4's table) | 215.12 |
| η=0 (no NRCC response) | 217.94 (+2.83) |
| η=1 (dollar-for-dollar mechanical matching) | 216.47 (+1.36) |
| **Nash equilibrium (NRCC as a genuine rational optimizer)** | **215.22 (+0.10)** |

Converged in 13 rounds. **Zero-sum sanity check passes exactly**: E[D seats] + E[R seats] = 215.22 + 217.78 = 433.00 = n_races. Multi-start agreement (`find_nash_equilibrium_multi_start()`, three independent starting points — observed baseline, uniform-across-races, zero-party-spend) confirms this is very likely a genuine, not-just-locally-found equilibrium: all three converged, and their final allocations agree to within \$8,566/race (D) and \$6,776/race (R) — an order of magnitude tighter than the \$10,000/race convergence tolerance itself, not a coincidence of one lucky starting point.

**This materially changes the practical reading of §9.4's table.** Both the η=0 and η=1 figures describe D unilaterally reoptimizing while R's reaction is either absent or a fixed mechanical rule R never chose strategically. Once R is modeled as *also* rationally reoptimizing — not just reacting — the DCCC's available edge over a genuinely rational NRCC nearly vanishes: +0.10 seats, not +1.36 to +2.83. This does not mean the underlying efficiency finding (§7, §8: DCCC *historically* under-invested where MSG was highest) is wrong — that is a retrospective fact about what DCCC actually did, independent of what NRCC would do in response to a *different* DCCC allocation. What it does mean is that the operational recommendation implied by §9.4 and §9.5 — that a *forward-looking* reallocation captures a robust, multi-seat edge even under adversarial response — was likely overstated by treating NRCC as non-strategic. A rational NRCC facing a reoptimizing DCCC gives almost all of that apparent edge back. This is the single most consequential correction to this document's actionability claims since the persuasion-ceiling fix (§7.3b).

**Caveats, stated rather than left implicit.** (1) The R-side ceiling mirror is unvalidated against NRCC-specific data, as above — if it is meaningfully wrong, the 215.22 figure could shift, though the zero-sum and multi-start checks at least confirm internal consistency of whatever model was actually solved. (2) SLSQP is a local solver on both sides; global optimality of the reported fixed point is not proven, only checked via three starting points agreeing (not a formal uniqueness proof). (3) The run is expensive (~50 minutes at full scale, 433 races × 13 rounds × 2 SLSQP solves/round) — not yet run for 2022 OOS or across a γ/cap sensitivity grid; both are natural follow-ups before treating 215.22 as a settled number rather than a first credible estimate. (4) `cap_fraction_r=0.15` (mirroring D's default) has no independent empirical basis for NRCC specifically.
...
### 10.5 Republican spending treated as fixed (Nash equilibrium closes this, 2026-08)

The base model takes Republican spending as given. §9 (Actionability) describes the operational implications, and §9.4 (Adversarial Response) quantifies the seat gain under adversarial NRCC matching via the η parameter. Even at η = 1 (dollar-for-dollar response), the optimizer yields +1.36 seats over DCCC (corrected 2026-07-24, persuasion-ceiling fix — was +7.07 under 2026-07-23b; +1.88 against an even earlier, stale placeholder baseline before that). However, the η model is a reduced-form approximation — it does not model the NRCC's *targeting* of counter-spending, only the total magnitude.

**A Nash equilibrium formulation, previously left as future work, is now implemented — see §9.4b.** NRCC is modeled as its own constrained optimizer (not a mechanical reaction rule), and the joint fixed point of both committees' simultaneous best responses is solved via best-response dynamics. Headline 2024 result: E[D seats] = 215.22 at the Nash equilibrium, essentially indistinguishable from DCCC's actual observed 215.12 — far closer to parity than either the η=0 (+2.83) or η=1 (+1.36) figures suggested, once NRCC is credited with genuine strategic rationality rather than either no response or a fixed mechanical one. §9.4b's caveats (R-side ceiling unvalidated against NRCC-specific data, local-solver optimality, not yet run OOS) still apply — this is a first credible estimate, not a fully settled one — but the qualitative reading (§9's operational recommendations were likely overstated against a truly rational opponent) is now backed by a real two-sided solve, not just the direction implied by a one-sided sensitivity sweep.
...
**Gap 1 — CVAP (spending intensity per voter) — attempted 2026-08, remains α₄ = 0.0.**  
...
**What was attempted.** A multi-vintage CVAP panel was fetched (`scripts/fetch_cvap_panel.py`, 9 Census CVAP special-tabulation vintages, 2016–2024 end-years — CD-level data doesn't exist before the 2016 vintage, verified directly: 2014/2015 only publish `MCD.csv`, no `CD.csv`) to exploit genuine within-district variation via fixed effects, rather than relying on the single static 2022 cross-section that made the original OLS attempt un-identifiable in the first place.

**A real, useful finding surfaced while building the panel, not assumed from documentation.** Census re-tabulates each CVAP vintage under whatever congressional map is *current at publication time*, not the map in effect during that vintage's underlying ACS collection years — verified empirically by diffing district_id sets across vintages: the 2021→2022 vintage boundary shows exactly the real 2020-census apportionment shift (TX 36→38 districts, MT 1→2, CA/IL/MI/NY/OH/PA/WV each −1), and every vintage from 2022 onward is otherwise identical to its neighbors. Practical consequence: (1) `district_id` is genuinely a different geography before vs. after this boundary, so a naive multi-vintage merge would silently mix incomparable districts; (2) there is *no* real within-district time variation among the four "post" vintages fetched (2021–2024) — they are the same map, re-tabulated, not four independent measurements.

**Redistricting-jump instrument: not attempted, scope boundary stated rather than faked.** The original plan's second identification strategy (using the mechanical CVAP jump at redistricting as a quasi-experimental instrument) requires a real geographic (GIS) crosswalk matching pre- and post-2022 district polygons by overlap — genuine spatial-analysis work this pass did not build. (For the record, since the redistricting-commission-state classification was researched as part of scoping this: the Brennan Center for Justice classifies Arizona, California, Colorado, and Michigan as using independent redistricting commissions, and Hawaii, Idaho, Montana, New Jersey, and Washington as using bipartisan political commissions, for 2020-cycle congressional maps — [Who Controlled Redistricting in Every State](https://www.brennancenter.org/our-work/research-reports/who-controlled-redistricting-every-state). These 9 states would be the natural "non-controversial redistricting" subsample if the GIS crosswalk is ever built; not used for anything in this pass since no instrument was constructed to apply it to.)

**What was delivered instead: a within-district fixed-effects estimate (not an IV estimate).** `src/backtest/estimation/cvap_iv.py` merges the historical margin-model panel (2012–2022, contemporaneous cycle→vintage mapping — a cycle uses the CVAP vintage whose window ends closest to that cycle's year, matching the timing convention `log_total_per_voter` already uses elsewhere) and fits `margin_pp ~ log_total_per_voter + log_ratio + gb` with district fixed effects via `linearmodels.PanelOLS` (`scripts/estimate_alpha4_iv.py`).

**Result** (real data, `python scripts/fetch_cvap_panel.py && python scripts/estimate_alpha4_iv.py`, saved to `outputs/alpha4_iv_attempt.json`):

| | value |
|---|---|
| α₄ (within-district FE) | −0.792 |
| SE | 0.790 |
| p-value | **0.317** |
| n observations | 2,452 (427 districts, up to 6 cycles each) |
| R² (within) | 0.213 |
| F-test for poolability | p < 0.0001 (real district heterogeneity confirmed — the FE specification itself is not vacuous) |

**Honest conclusion: still no usable α₄.** The FE point estimate has the same *sign* as the original naive OLS (−2.02) but is roughly a quarter the magnitude and is not remotely statistically significant (p=0.32, nowhere near conventional thresholds) — indistinguishable from zero given the noise. This is a *different* finding from the original OLS result, not a confirmation of it: the original problem (endogenous cross-sectional correlation between spending intensity and competitiveness) is genuinely addressed by the fixed-effects design, but the available within-district signal — mostly coming from four to five overlapping, heavily-autocorrelated pre-redistricting ACS windows (2016–2021) plus a single post-redistricting cross-section (2022) — is too weak to pin down a reliable coefficient either direction. This is the exact "instrument search attempted, honestly documented, did not yield a usable estimate" outcome flagged as a legitimate possibility going in.

**α₄ remains 0.0 in `data/processed/margin_model_coef.json` — not changed.** Per this project's standing practice for consequential coefficient changes (the original naive OLS attempt was itself rejected only after a manual OOS Brier check, not adopted on point-estimate sign alone), a p=0.32 FE result would not clear that bar regardless. Tests: `tests/test_cvap_iv.py` (9 tests, including a positive-control synthetic-DGP recovery check confirming the estimator itself is implemented correctly, independent of whether the real data's signal is strong enough).
...
**Gap 3 — State party coordinated expenditures — CLOSED 2026-08.** FEC "Other Transactions" (24K filings) include committee-to-committee transfers, a subset of which are state Democratic party coordinated expenditures into House races not attributed to the DCCC. Originally documented above (and, before this fix, in this section) as out-of-scope future work, at the wrong path (`data/raw/bulk_all/itoth.txt` — that file doesn't exist). The real file is `data/raw/all_committee_transactions/itoth.txt` (167MB, ~1.01M rows), plus 7 undocumented sibling files (`itoth 2.txt`…`itoth 8.txt`, 90MB–3.3GB, overlapping vintage snapshots of the same bulk product, verified directly rather than assumed from filenames) — 7.86GB combined.

Implementation: `scripts/fetch_data.py`'s `_load_committee_master()`, `identify_state_dem_party_committees()`, `parse_state_party_coordinated_24k()`; tests in `tests/test_fetch_data_state_party.py` (18 tests). Committee identification required a whitelist-style check (own state's full name + a structural party-committee suffix pattern, plus a small manually-verified override list for 4 real state parties whose registered names don't spell out their state or use no "DEMOCRATIC"/"PARTY" text at all — e.g. Georgia's federal party committee is literally named "GEORGIA FEDERAL ELECTIONS COMMITTEE") — a naive `CMTE_TP`/`CMTE_DSGN`/`CMTE_PTY_AFFILIATION`-only filter was tried first and incorrectly admitted 235 committees (town/county Democratic clubs, caucuses, legislative-district organizations, joint-fundraising trusts) before being tightened; the final version correctly identifies 49 of 50 states' Democratic party committees (Indiana's closest candidate is an ambiguous "victory committee," left out rather than guessed) plus DC out of scope for this project's House-race universe regardless.

**2024 result** (`python scripts/fetch_data.py --only state-party-coordinated --cycles 2024`, ~94s wall clock scanning the 3 relevant itoth files for the 2023–2024 window): **$153,106 across 39 districts** — confirming the likely-small-relative-to-IEs expectation this section originally stated. Full per-district breakdown in `data/raw/fec/coordinated_state_party_dem_2024.csv`; largest single districts were PA-10 ($14,373) and TX-33 ($11,500). A clean before/after `d_total` sanity check (`build_universe(2024)`, summed across all 433 races) confirmed the total shifted by exactly $153,106 with no other district's data disturbed.

**A real bug found and fixed while wiring this in**: `src/backtest/data/fec.py`'s `build_total_spend()` assumed exactly one `coordinated_expenditures` row per (district, party) — true when DCCC and NRCC were the only two coordinated-spend sources (disjoint parties), but false once a district could receive coordinated money from *both* DCCC and a state party (both party="D"). The bug crashed outright (`pd.concat` "cannot reindex on an axis with duplicate labels") rather than silently double- or under-counting, so it was caught immediately rather than shipping a wrong number; fixed by summing via `groupby("district_id")` instead of `set_index()`. Test: `tests/test_fec.py::TestBuildTotalSpend::test_two_coordinated_rows_for_the_same_district_and_party_are_summed_not_overwritten`.

**A second, more consequential incident, caught and reverted before it reached any reported number**: wiring the new source in via `consolidate_fec_files(cycle, force=True)` also silently re-ran that function's separate "ie" (independent-expenditure) consolidation branch, overwriting the existing, richer `build_comprehensive_ie()` output (`independent_expenditures_2024.csv`, ~350 rows spanning many outside groups) with a much narrower DCCC/NRCC-API-only concatenation — corrupting real spending data on the very first live run. A ~$358M drop in total `d_total` from what should have been a ~$153K addition was not remotely plausible and was caught immediately by the same before/after sanity check described above, before it touched any committed output; the file was restored from git, and `consolidate_fec_files()` gained a `kinds` parameter (`kinds=["coordinated"]`) so a caller adding a new *coordinated* source can no longer accidentally force-refresh the unrelated *IE* file. Regression tests: `tests/test_fetch_data_state_party.py::TestConsolidateFecFilesKindsScoping`. This is recorded here deliberately, not just fixed silently — a $358M miscount would have been a serious, hard-to-detect corruption of every downstream headline number in this document had the sanity check not caught it before anything was committed.

Only 2024 has been run so far; 2022 and the remaining panel cycles (2012–2020) are a natural follow-up (`python scripts/fetch_data.py --only state-party-coordinated --cycles 2022 2020 2018 2016 2014 2012`), not yet done.
